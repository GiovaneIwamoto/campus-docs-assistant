...
def configure_sidebar() -> dict:
    ...
    with st.sidebar:
        ...
        pinecone_index_name = keys_expander.text_input("Pinecone Index Name", value=st.secrets["pinecone_index_name"], disabled=True)
        
        # Support for Ollama or OpenAI embedding models
        embedding_model_provider = st.secrets["embedding_model_provider"]
        embedding_model = st.secrets["embedding_model"]
        openai_api_key = st.secrets["openai_api_key"]
        
        # Disable this section for deployment, as keys are managed via Streamlit secrets
        ...
        web_url = index_expander.text_input("Web Link", placeholder="https://example.com", disabled=True)
        web_indexing_enabled = index_expander.button("Activate Web Indexing", icon=":material/database_upload:")

        # File indexing section
        uploaded_files = index_expander.file_uploader("File Upload", type=["pdf", "txt", "docx", "zip"], accept_multiple_files=True, disabled=True)
